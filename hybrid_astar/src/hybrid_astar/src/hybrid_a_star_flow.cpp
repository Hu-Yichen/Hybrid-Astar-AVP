...
HybridAStarFlow::HybridAStarFlow(ros::NodeHandle &nh) {
    ...
    path_pub_ = nh.advertise<nav_msgs::Path>("searched_path_", 1);
    ori = nh.advertise<hybrid_astar::path>("ori", 1);
...
void HybridAStarFlow::PublishPath(const VectorVec3d &path) {
    ...
    hybrid_astar::path oripath; //creat message for 3d path
    hybrid_astar::path update; //creat temp message to update oripath

    if (!path.empty()) {
        // Populate the fields of the custom message with elements from the first path element
        const Eigen::Matrix<double, 3, 1> &first_path_element = path[0];
        oripath.x = first_path_element(0);
        oripath.y = first_path_element(1);
        oripath.z = first_path_element(2);
    }
    for (const auto &pose: path) {
        pose_stamped.header.frame_id = "world";
        pose_stamped.pose.position.x = pose.x();
