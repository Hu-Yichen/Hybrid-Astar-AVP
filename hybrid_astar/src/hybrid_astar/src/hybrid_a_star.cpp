...
#include "hybrid_astar/display_tools.h"
#include "hybrid_astar/timer.h"
...
void HybridAStar::Init(double x_lower, double x_upper, double y_lower, double y_upper,
    ...
    // std::cout << MAP_GRID_SIZE_X_ << std::endl;
    // std::cout << MAP_GRID_SIZE_Y_ << std::endl;
...
double HybridAStar::ComputeH(const StateNode::Ptr &current_node_ptr,
    ...
    // constrict
    // h = (gmap[int((current_node_ptr->state_.y() - map_y_lower_) / STATE_GRID_RESOLUTION_)][int((current_node_ptr->state_.x() - map_x_lower_) / STATE_GRID_RESOLUTION_)])*STATE_GRID_RESOLUTION_;
    
    // L2 norm & Reeds
    // h = (current_node_ptr->state_.head(2) - terminal_node_ptr->state_.head(2)).norm();
    // if (h < 1.5 * shot_distance_) {
    ...
    // int x = std::min(std::max(int((current_node_ptr->state_.x() - map_x_lower_) / STATE_GRID_RESOLUTION_), 0), STATE_GRID_SIZE_X_ - 1);
    // int y = std::min(std::max(int((current_node_ptr->state_.y() - map_y_lower_) / STATE_GRID_RESOLUTION_), 0), STATE_GRID_SIZE_Y_ - 1);
    // h = (gmap[y][x])*STATE_GRID_RESOLUTION_;
...
bool HybridAStar::Search(const Vec3d &start_state, const Vec3d &goal_state) {
    ...
    map_display(STATE_GRID_SIZE_X_, STATE_GRID_SIZE_Y_, goal_grid_index);
    std::cout << "start position:" << std::endl;
    std::cout << start_grid_index[0] << std::endl;
    std::cout << start_grid_index[1] << std::endl;
    std::cout << "goal position:" << std::endl;
    std::cout << goal_grid_index[0] << std::endl;
    std::cout << goal_grid_index[1] << std::endl;
...
void HybridAStar::map_display(unsigned int width, unsigned int height, Vec3i goal_grid_index){
    const int INF = 7; // Infinity value for unreachable cells
    gmap.resize(height);
    for (int i = 0; i < gmap.size(); i++) {
        gmap[i].resize(width, INF);
    }
    for(unsigned int i = 0; i<height; ++i){
        for(unsigned int j = 0; j<width; ++j){
            int w = std::floor(j/MAP_GRID_RESOLUTION_);
            int h = std::floor(i/MAP_GRID_RESOLUTION_);
            if(map_data_[w + h * MAP_GRID_SIZE_X_] != 1)
                gmap[i][j] = 0;
        }
    }
    ...
    // Vec3i goal_grid_index = State2Index(goal_state);
    int goalX = goal_grid_index[0];
    int goalY = goal_grid_index[1];
    // BFS queue
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int w = std::floor(newX/MAP_GRID_RESOLUTION_);
            int h = std::floor(newY/MAP_GRID_RESOLUTION_);
            if (newX >= 0 && newX < cols && newY >= 0 && newY < rows && map_data_[w + h * MAP_GRID_SIZE_X_] != 1 && !(newX == goalX && newY == goalY)) {
                // Update the distance if it's shorter
                if (gmap[newY][newX] == 0) {
                    gmap[newY][newX] = gmap[y][x] + 1;
                    q.push({newX, newY});
                }
            }
        }
    }
    // for(int y =gmap.size()-1; y >= 0; y--){
    //     for(int x = 0; x<60; x++){
    //         if(gmap[y][x]<10)
    //             std::cout << gmap[y][x] << " ,"; 
    //         else if(gmap[y][x]<100)
    //             std::cout << gmap[y][x] << ","; 
    //         else
    //             std::cout << gmap[y][x]/10<< ",";
    //     }
    //     std::cout << ";" << std::endl;
    // }
    return;
...
// void calculateShortestDistances(const std::vector<std::vector<int>>& grid, int goalX, int goalY, std::vector<std::vector<int>>& distance) {
